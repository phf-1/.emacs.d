...
1. user--org-remove-property-drawers : Function that executes after tangling.
...
- org-babel-spec-to-string : like org-babel-spec-to-string but removes new lines.
...
1. init.bash : File that contains Bash environment variable declarations.
...
1. guix-packages.scm : File that evaluates to a list of Guix packages.
...
1. locale : Guix package that provides utf8 locale.
...
1. elisp-packages : List of Elisp guix packages.
...
1. binaries : List of Guix packages that provides binaries.
...
1. configuration : List of parameters that control the behavior of the Makefile.
...
1. BUILD : variable that refers to the build directory.
...
1. SRC : variable that refers to the source file.
...
1. PACKAGES : variable that refers to the list of Guix packages of the environment.
...
1. INIT : variable that refers to init file of the bash session in the environment.
...
1. ELISP : variable that refers to ELisp code that should be loaded by ${EMACS}.
...
1. EMACS : variable that refers to the Emacs executable.
...
1. GUIX : variable that refers to guix executable.
...
1. ENVIRONMENT : List of files that form the development environment.
...
1. init : Makefile target that tangles necessary files.
...
1. ~clean~ : Makefile target that delete all generate files necesseary files.
...
1. ~env~ : Makefile target that starts the environment.
...
1. elc : Makefile target that byte compiles the package.
...
1. ~lint~ : Makefile target that lints the package.
...
1. ~lint~ : Makefile target that check the docstrings of the package.
...
1. ~all~ : Makefile target that builds, lint and checkdoc the package.
