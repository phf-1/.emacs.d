#+PROPERTY: header-args:emacs-lisp :noweb yes :tangle locs-and-refs.el :comments org
...
EMACS := emacs -Q --batch
DEPS := ${HOME}/.emacs.d/elpa
ORG_PATCHES := ${HOME}/.emacs.d/elisp/org-patches.el
...
.PHONY: el
el: locs-and-refs.el
locs-and-refs.el: locs-and-refs.el.org
	$(EMACS) $< \
		-l $(ORG_PATCHES) \
		-f org-babel-tangle
	sed -i '1,2d' $@
...
.PHONY: elc
elc: locs-and-refs.elc
locs-and-refs.elc: locs-and-refs.el
	$(EMACS) -L $(DEPS)/pcre2el-20240629.2322 \
		-f batch-byte-compile locs-and-refs.el

.PHONY: checkdoc
checkdoc: checkdoc-report.txt
checkdoc-report.txt: locs-and-refs.el
	$(EMACS) --eval '(checkdoc-file "$<")' > $@

.PHONY: lint
lint: lint-report.txt
lint-report.txt: locs-and-refs.el
	$(EMACS) $< \
		-L $(DEPS)/package-lint-20250108.1407 \
		-l package-lint \
		-f package-lint-buffer > $@

.PHONY: clean
clean:
	rm -fv locs-and-refs.el
	rm -fv locs-and-refs.elc
	rm -fv checkdoc-report.txt
	rm -fv lint-report.txt
#+end_src
