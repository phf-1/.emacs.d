...
(defun total-recall--Planner-init (db)
  (let ((memory (total-recall--Actor-memory
...
(defun total-recall--Rating-init (data)
  (let ((memory (total-recall--Actor-memory
                 #'total-recall--Rating-compiler
                 #'total-recall--Rating-interpreter)))
    memory))

(defun total-recall--Rating-compiler (msg)
  (pcase msg
    (_ (error "Unexpected msg. msg = %s" msg))))

(defun total-recall--Rating-interpreter (memory inst)
  (let ((self (gethash 'self memory)))
    (pcase inst
      (_ (error "Unexpected instruction. inst = %s" inst)))))
#+end_src
...
(defun total-recall--UI-interpreter (memory inst)
  (let ((self (gethash 'self memory)))
    (pcase inst
      (`(show exercise ,exercise)
       ;; TODO
       (puthash 'out `(rating ,(total-recall--Rating t)) memory))
    ...
    (when (not inst-are-ignored)
      (pcase inst
        ('stop
         ...
         (total-recall--show ui 'report report)
        ...
        (`(process-exercise ,exercise)
         ...
         (pcase (total-recall--show ui 'exercise exercise)
           ('stop (total-recall--stop self))
           (`(rating ,rating)
