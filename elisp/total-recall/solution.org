...
~clock#now()~ is a string satisfying ISO8601 representing the time at the call.
...
(defun total-recall--Clock-exec (memory inst)
  (let ((time (gethash 'time memory)))
    ...
    (pcase inst
      ...
      ('now
       (puthash 'out (format-time-string "%FT%TZ" (time-convert (current-time) 'list) t) memory))
...
date ~date~, an ISO8601 string, with the value ~value~, a symbol.
...
(defun total-recall--Rating-exec (memory inst)
  (let ((self (gethash 'self memory))
    ...
    (pcase inst
      ('strings
       (puthash 'out `(,date ,id ,(symbol-name value)) memory))
      (_ (error "Unexpected instruction. inst = %s" inst)))))
#+end_src
...
(defun total-recall--DB-exec (memory inst)
  (let ((self (gethash 'self memory)))
    (pcase inst
      (`(save ,rating)
       (message "%s" (total-recall--strings rating))
       (total-recall--not-implemented-warning)
       (puthash 'out self memory))
...
(defun total-recall--UI-init (data)
  (let ((memory (total-recall--Actor-memory #'total-recall--UI-rcv #'total-recall--UI-exec)))
    (pcase data
...
(defun total-recall--UI-exec (memory inst)
  (let ((self (gethash 'self memory))
    ...
    (pcase inst
      (`(show-exercise ,exercise)
       (total-recall--UI-exec memory 'show-frame)
       ...
       (total-recall--UI-exec memory '(ask ((?1 . "Stop") (?2 . "Skip") (?3 . "Reveal"))))
       (pcase (gethash 'out memory)
         (?1 (total-recall--UI-exec memory 'stop))
         (?2 (total-recall--UI-exec memory `(skip ,exercise)))
         (?3
          (total-recall--UI-exec memory `(show-content ,(total-recall--answer exercise)))
          (total-recall--UI-exec memory '(ask ((?1 . "Success") (?2 . "Failure") (?3 . "Skip") (?4 . "Stop"))))
          (pcase (gethash 'out memory)
            (?1 (total-recall--UI-exec memory `(success ,exercise)))
            (?2 (total-recall--UI-exec memory `(failure ,exercise)))
            (?3 (total-recall--UI-exec memory `(skip ,exercise)))
            (?4 (total-recall--UI-exec memory 'stop))))))
      ...
      (`(show-report ,report)
       (total-recall--UI-exec memory 'clear)
       (total-recall--UI-exec memory 'show-frame)
       (total-recall--UI-exec memory `(show-content ,(total-recall--string report)))
       (puthash 'out self memory))
    ...
    (pcase inst
      ('start
       (let ((nbr-files (puthash 'nbr-files 0 memory))
         ...
         (total-recall--TotalRecall-exec memory 'process-files)
      ...
      (`(process-file ,file)
       ...
       (puthash 'exercises (total-recall--parse parser file) memory)
       (total-recall--add report (format "%s exercises have been found." (length (gethash 'exercises memory))))
       (total-recall--TotalRecall-exec memory 'process-exercises)
