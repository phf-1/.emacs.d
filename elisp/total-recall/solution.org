...
(defun total-recall--Searcher-init (time)
  (let ((memory (total-recall--Actor-memory
...
~io#minibuffer(s)~ writes the string ~s~ to Emacs minibuffer.
  ...
  (let ((self (gethash 'self memory))
    ...
    (pcase inst
	 ...
	 (insert (string-join (list string "\n"))))
      ...
      ('buffer-name
       (puthash 'out (gethash 'name memory) memory))
  ...
  (pcase data
    (`(,root ,db-path)
     (let ((memory (total-recall--Actor-memory
       ...
       (puthash 'searcher (total-recall--Searcher root) memory)
