...
(defcustom trl-exercise-type-id "b0d53cd4-ad89-4333-9ef1-4d9e0995a4d8"
  ...
  (pcase msg
    (:list-exercises
     (mapcan
      ...
      (trl--search-rcv `(:search ,dir "org" ,trl-exercise-type-id))))))
#+end_src
...
1. path
2. files :≡ [path/**/file.org such that ∃ heading with type = exercise id]
3. path/**/file.org → [heading with type = exercise id]
4. heading → path id question answer
   1. path ≡ file.org/xxx/yyy
...
(defun total-recall ()
  ...
  (let ((exercises (trl--path-rcv trl-root-dir :list-exercises))
    ...
    (run-at-time 2 nil (lambda () (trl--ui-rcv ui :kill)))))
#+end_src
