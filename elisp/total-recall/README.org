...
(cl-defstruct
    (trl--point
     (:constructor trl--point-mk
		   (x y
		      &aux
		      (_ (unless (integerp x)
			   (error "X must be an integer: %s" x)))
		      (_ (unless (integerp y)
			   (error "Y must be an integer: %s" y))))))
  x
  y)
...
(f pt) ≡ (func (point-x pt) (point-y pt))"
  (lambda (pt)
    (unless (trl--point-p pt) (error "PT is not a Point"))
    (funcall func (trl--point-x pt) (trl--point-y pt))))
...
#+begin_src emacs-lisp
(let (pt-a pt-b)
  (setq pt-a (trl--point-mk 10 20))
  (unless (trl--point-p pt-a) (error))
  (unless (eq (trl--point-x pt-a) 10) (error))
  (unless (eq (trl--point-y pt-a) 20) (error))
  (trl--point-func "trl--point-add" (lambda (x y) (+ x y)))
  (unless (eq (trl--point-add 1) 30) (error)))
#+end_src
...
#+begin_src makefile
.PHONY: el
LOCS_AND_REFS_EL := ${BUILD}/total-recall.el
el: ${LOCS_AND_REFS_EL}
...
#+begin_src makefile
.PHONY: elc
LOCS_AND_REFS_ELC := ${BUILD}/total-recall.elc
