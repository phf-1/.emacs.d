...
#+begin_src emacs-lisp
;; Built-in Completion Preview (Emacs 29+)
(require 'completion-preview)

;; Enable in almost all text-editing modes
(dolist (hook '(prog-mode-hook
                text-mode-hook
                conf-mode-hook
                comint-mode-hook          ; shell, eshell, ielm, etc.
                org-mode-hook))
  (add-hook hook #'completion-preview-mode))

;; Core settings
(setq completion-preview-minimum-symbol-length 2)     ; don’t be annoying
(setq completion-preview-exact-match-only nil)    ; show preview even with many candidates
(setq completion-preview-insert-on-completion t)  ; TAB just accepts (like VS Code)

;; Show the preview immediately while you type (this is the magic)
...
;; Key bindings – exactly what you expect
...
;; Feed Completion Preview with modern, fast, case-respecting sources
(require 'cape)

;; Global sources (cape-dabbrev is the new, better dabbrev)
(add-to-list 'completion-at-point-functions #'cape-dabbrev)
(add-to-list 'completion-at-point-functions #'cape-file)
(add-to-list 'completion-at-point-functions #'cape-keyword)
(add-to-list 'completion-at-point-functions #'cape-history)     ; great in shells

;; Push cape-dabbrev to the front in programming and shell modes (fastest & most relevant)
...
;; Never again get 1-API-DRAFT when you wrote 1-api-draft
(setq dabbrev-case-replace nil)          ; old dabbrev fix (harmless if you never use it)
(setq cape-dabbrev-check-other-buffers t)  ; scan all buffers, like dabbrev used to

;; Modern fuzzy-ish completion everywhere (optional but 99 % of people love it)
...
Inline completion preview using the built-in Completion Preview mode.

#+begin_src emacs-lisp
(require 'completion-preview)

;; Enable in almost all text-editing modes (mirrors your original choice)
(dolist (hook '(prog-mode-hook
                text-mode-hook
                conf-mode-hook
                comint-mode-hook          ; shell, eshell, ielm, etc.
...
(setq completion-preview-minimum-symbol-length 2)     ; avoid noise on short symbols
(setq completion-preview-exact-match-only nil)        ; show preview even with multiple candidates
(setq completion-preview-insert-on-completion t)      ; TAB accepts the preview

;; Automatically show the preview while typing in common situations
(setq completion-preview-commands
      '(self-insert-command
        delete-backward-char
        backward-delete-char-untabify
        org-self-insert-command
        org-delete-backward-char
        paredit-backward-delete))  ; add more if you use other structural editing modes

;; Keybindings in the active preview
(define-key completion-preview-active-mode-map (kbd "TAB")   #'completion-preview-insert)
(define-key completion-preview-active-mode-map (kbd "<tab>") #'completion-preview-insert)
(define-key completion-preview-active-mode-map (kbd "M-n")   #'completion-preview-next-candidate)
(define-key completion-preview-active-mode-map (kbd "M-p")   #'completion-preview-prev-candidate)
...
;; Manual trigger (your existing preferred keys) – this restarts/re-shows the preview
...
** Contextual menu
Contextual actions for completions, buffers, files, etc.

#+begin_src emacs-lisp
(require 'embark)
(require 'embark-consult)  ; essential integration with Consult

;; Default action key — C-. opens the Embark action menu
(global-set-key (kbd "C-.") #'embark-act)

;; Show available keybindings for the current target
(global-set-key (kbd "C-h B") #'embark-bindings)

;; Live-updating preview of actions as you move through candidates
(setq embark-live t)

;; Make Embark act on the region when active (very useful)
