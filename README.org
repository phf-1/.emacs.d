...
#+begin_src emacs-lisp
(add-to-list 'load-path (file-name-concat user--package "locs-and-refs/src"))
(require 'lar)
#+end_src
...
(setq org-cite-global-bibliography
      (list (expand-file-name "~/src/website/bibliography.bib")))
...
(defun org-copy-outline-path-to-kill-ring ()
  ...
  (let* ((filename (file-name-nondirectory (buffer-file-name)))
         ...
         (heading-with-todo (if todo-keyword
                              ...
                              current-heading))
         (full-path (append (list filename) outline-path (list heading-with-todo)))
...
;; (remove-hook 'org-babel-post-tangle-hook #'user--org-remove-property-drawers)
...
(defun my/org-agenda-prefix-filename ()
  "Return just the filename without path for agenda prefix."
  ;; (if (buffer-file-name (org-base-buffer (current-buffer)))
  ;;     (concat (file-name-nondirectory (buffer-file-name (org-base-buffer (current-buffer)))) "->")
  ;;   "")
  "")

(setq org-agenda-prefix-format
      '((agenda . "%s %-12t %(my/org-agenda-prefix-filename)%b")
        (todo   . "%(my/org-agenda-prefix-filename)%b")
        (tags   . "%(my/org-agenda-prefix-filename)%b")
        (search . "%(my/org-agenda-prefix-filename)%b")))
...
(defun user--org-export-current-buffer-to-utf8-buffer ()
  "Export current org buffer to *Org UTF-8 Export* buffer."
  (let ((translated (org-export-as 'ascii nil nil nil '(:ascii-charset utf-8)))
        (buffer (get-buffer-create "*UTF8 export*")))
    (with-current-buffer buffer
      (erase-buffer)
      (insert translated))))

(defun user--org-export-current-buffer-to-utf8-file ()
  "Export current org buffer to *Org UTF-8 Export* file."
...
(org-link-set-parameters "ref"
  :export (lambda (path desc backend)
            (pcase backend
              ('ascii desc)
              ('utf8 desc)
              ('html (format "<span class=\"ref\">%s</span>" desc))
              (_ desc))))
...
(org-link-set-parameters "id"
  :export (lambda (path desc backend)
            (pcase backend
              ('ascii desc)
              ('utf8 desc)
              ('html (format "<span class=\"id\">%s</span>" desc))
              (_ desc))))
#+end_src

*** Time may be measured per task and summarized for a whole plan
Given a plan:
#+begin_src org
,* Plan
,** TODO Objective
...
Then ~C-c i/o~ starts/stops a clock for the heading at point.
#+begin_src emacs-lisp
(with-eval-after-load 'org
...
Then, a detailed summary may be deduced with ~C-c p~ when point on some heading. ~C-c m~
to remove it.
#+begin_src emacs-lisp
(with-eval-after-load 'org
  (define-key org-mode-map (kbd "C-c p") #'org-clock-display)
  (define-key org-mode-map (kbd "C-c m") #'org-clock-remove-overlays))
#+end_src
...
(let (xai-backend)
  ...
  (setq gptel-backend xai-backend
        gptel-model 'grok-code-fast-1)

  (add-hook 'gptel-mode-hook
            (lambda ()
              (when (project-current)
                (gptel-add (project-root (project-current)))))))
#+end_src
...
curl https://api.x.ai/v1/chat/completions \
  ...
  -H "Authorization: Bearer xai-your-full-key-here" \
  -d '{
