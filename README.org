...
(add-to-list 'eglot-server-programs
             `(elixir-mode
               ,(file-name-concat (expand-file-name "~") "bin" "elixir-ls" "language_server.sh")))
#+end_src
...
(use-package org
  ...
  :custom
  (org-cite-global-bibliography (list (expand-file-name "~/src/bibliography/reference.bib")))
  (org-cite-csl-styles-dir (expand-file-name "~/.emacs.d/csl"))
  ...
  ;; For whatever reason, this is not defined.
  (defun citeproc-style-cite-superscript-p (x)
    t)
  ...
  (defun org-show-all-drawers ()
    "Show all drawers in the current Org buffer."
    (interactive)
    (org-element-map (org-element-parse-buffer) 'property-drawer
      (lambda (drawer)
        (org-fold-hide-drawer-toggle 'off nil drawer))))
...
* auto replace characters

#+begin_src emacs-lisp
;;; Replace "->" with "→" as you type (works inside words and in all major modes)

(defcustom arrow-replace-include-minibuffer nil
  "If non-nil, perform arrow replacement in the minibuffer too.
Default nil because changing text in the minibuffer can interfere with
completions and special minibuffer behavior."
  :type 'boolean
  :group 'convenience)

(defun arrow-replace--post-self-insert ()
  "Replace the two characters \"-\" and \">\" before point with the single char \"→\".
Intended to be added to `post-self-insert-hook'."
  (when (and
         ;; optionally skip minibuffer
         (or arrow-replace-include-minibuffer (not (minibufferp)))
         ;; don't touch read-only buffers
         (not buffer-read-only)
         ;; need at least two chars
         (>= (point) 2)
         ;; last typed char is '>' and the one before that is '-'
         (eq (char-before) ?>)
...
(define-minor-mode arrow-replace-mode
  "Buffer-local minor mode: replace \"->\" with \"→\" as you type."
  :lighter " →"
  :global nil
  (if arrow-replace-mode
      (add-hook 'post-self-insert-hook #'arrow-replace--post-self-insert nil t)
    (remove-hook 'post-self-insert-hook #'arrow-replace--post-self-insert t)))

(define-globalized-minor-mode global-arrow-replace-mode arrow-replace-mode
  ;; Init function: enable arrow-replace-mode in new buffers (respect minibuffer setting)
  (lambda ()
    (when (or arrow-replace-include-minibuffer (not (minibufferp)))
      (arrow-replace-mode 1))))

;; Enable it globally:
(global-arrow-replace-mode 1)
#+end_src
